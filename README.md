...
GitHub's Markdown parser can be notoriously strict with LaTeX, especially when block math (`$$`) is nested inside bullet points or lacks exact blank-line spacing. 

Here is the corrected `README.md`. I have restructured the math blocks to sit in their own paragraphs (outside of lists) and enforced strict blank-line spacing around the `$$` delimiters, which guarantees they will render correctly on GitHub.
...
$$
P(w, z \mid \alpha, \beta) = P(w \mid z, \beta) P(z \mid d, \alpha)
$$
...
### 💡 Key Fixes Applied for GitHub Rendering:
1. **Removed Math from Lists:** GitHub's parser frequently breaks LaTeX blocks if they are indented inside bullet points. I moved all equations out of the lists and into standard paragraphs.
2. **Strict `$$` Spacing:** Block math now requires a completely blank line before and after the `$$` tags. 
3. **Multi-line Block Formatting:** Instead of `$$ equation $$`, I used the multiline format:
   ```latex
   $$
   equation
   $$
   ```
   This prevents the parser from confusing the closing `$$` with inline text.
4. **Replaced `|` with `\mid`:** In the LDA probability equation, changed the pipe character to `\mid` to ensure proper LaTeX spacing and prevent markdown parser conflicts.GitHub's Markdown parser can be notoriously strict with LaTeX, especially when block math (`$$`) is nested inside bullet points or lacks exact blank-line spacing. 
...
3. **Multi-line Block Formatting:** Instead of `$$ equation $$`, I used the multiline format:
   ...
   ```
   This prevents the parser from confusing the closing `$$` with inline text.
4. **Replaced `|` with `\mid`:** In the LDA probability equation, changed the pipe character to `\mid` to ensure proper LaTeX spacing and prevent markdown parser conflicts.
